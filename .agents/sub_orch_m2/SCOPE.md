...
1. **Eliminate All Test Auth Bypasses (Feature 6)**:
   - `src/auth.ts`: Remove lines 114–132 inspecting `x-test-bypass`. Export `auth` directly from `NextAuth`.
   - `src/auth.config.ts`: Remove lines 11–13 checking `x-test-bypass`.
   - `src/app/(authenticated)/board/[id]/page.tsx`: Remove lines 22–23 and 37–47 checking `isTestBypass` and synthesizing mock boards. Also ensure `isNull(boards.deletedAt)` is checked on the board query.
...
3. **Eliminate Token & Credential Leakage (Features 8, 9, 10, 11)**:
   - `POST /api/auth/signup`: Remove `verificationToken` from JSON response (line 78). Update `src/app/signup/page.tsx:81` to redirect to `/signup/verify?email=${encodeURIComponent(formData.email)}` without `token`.
   - `POST /api/auth/forgot-password`: Remove `resetToken: token` from line 45. Return `{ success: true, message: 'If an account exists with this email, a password reset link has been sent' }`.
...
7. **File Upload Hardening (Feature 15)**:
   - In `src/app/api/upload/route.ts`:
     ...
     - Verify magic bytes of the file buffer (JPEG, PNG, GIF, WEBP).

## Orchestration Procedure for M2 Sub-Orchestrator
Assess -> Run Iteration Loop (2B):
- Spawn Explorers (`teamwork_preview_explorer`) to plan code modifications.
- Spawn Worker (`teamwork_preview_worker`) with the MANDATORY INTEGRITY WARNING to implement changes and run tests.
- Spawn 2 Reviewers (`teamwork_preview_reviewer`) to independently review auth bypass elimination, SSRF protection, credential stripping, rate limiting, and IDORs.
- Spawn 2 Challengers (`teamwork_preview_challenger`) to probe `x-test-bypass` (expect 401), SSRF (127.0.0.1, 169.254.169.254, RFC1918 expect 400), signup/forgot-password token absence, `/api/me` passwordHash absence, and rate limiting concurrency.
- Spawn Forensic Auditor (`teamwork_preview_auditor`) to verify zero backdoors and authentic security controls.
- Gate evaluation in `GATE_STATUS.md`. All must PASS.
